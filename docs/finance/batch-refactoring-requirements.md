...
#### SQS
- メッセージ数 (簡易計算): 
  - 1分毎: 200件 × 60回 = 12,000メッセージ/時間
  - 10分毎: 300件 × 6回 = 1,800メッセージ/時間  
  - 1時間毎: 500件 × 1回 = 500メッセージ/時間
  - 合計: 約14,300メッセージ/時間 = 343,200メッセージ/日
- Standard Queue: 最初の100万リクエスト無料、以降も月額$0.40/100万リクエスト程度
- 月額: 約 $0.14 (月間約1,000万リクエスト)
...
- 稼働時間: 24時間 (常時起動ではなく、ジョブ処理時のみ)
  - 実質稼働: 343,200ジョブ/日 × 5秒 = 1,716,000秒/日 ≈ 28,600分/日
  - タスク並列度を考慮: 28,600分 / 3タスク ≈ 9,533分/日/タスク → 約200時間/日 (3タスク合計で約600時間/日)
- vCPU 料金: $0.04048/vCPU-時間
- メモリ料金: $0.004445/GB-時間
- 月額 (vCPU): 200時間/日 × 30日 × 0.25vCPU/タスク × $0.04048 ≈ $607
- 月額 (メモリ): 200時間/日 × 30日 × 0.5GB/タスク × $0.004445 ≈ $134

**注**: 上記は全ジョブを処理する場合の概算。実際には通知頻度により大幅に変動。

**合計: 約 $743/月 (Fargate 通常料金)**

**注**: このコストは全ジョブを毎回処理する最大ケースの見積もり。実際には：
- 多くの通知設定は条件を満たさず早期終了
- 頻度フィルタリングにより処理対象が削減される
- 実質的なコストは大幅に低くなる見込み ($50-100/月程度)
...
| 項目 | 現行 (Lambda) | 新 (Fargate) 最大 | 新 (Fargate) 実質 | 新 (Spot) 実質 |
|------|--------------|-----------------|----------------|---------------|
| Orchestrator | - | $1.44 | $1.44 | $1.44 |
| SQS | - | $0.14 | $0.14 | $0.14 |
| Compute | $34.56 | $741 | $98 | $29 |
| **合計** | **$35** | **$743** | **$100** | **$31** |

**結論:**
- 現行のユーザー規模では Lambda の方がコスト効率が良い可能性がある
- ただし、ユーザー数が増加すると ECS の方がスケーラビリティとコスト面で有利
- Fargate Spot 利用により、中規模でもコスト競争力を持つ
- **主な価値**: コストよりもスケーラビリティ、可用性、運用性の向上
