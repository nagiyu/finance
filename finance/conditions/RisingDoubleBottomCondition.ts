...
export const RisingDoubleBottomConditionInfo: ConditionInfo = {
  name: '切り上げダブルボトム',
  description:
    '切り上げダブルボトムは、下降トレンドの底値圏で出現する反転型のテクニカルパターンです。通常のダブルボトムとは異なり、2回目の安値が1回目よりも高い位置にあることが特徴で、下降トレンドの勢いが弱まり、買い圧力が強まっていることを示唆します。2つの底の間にできる戻り高値（ネックライン）を上抜けることで上昇トレンドへの転換シグナルとされます。',
  isBuyCondition: true,
...
export default class RisingDoubleBottomCondition extends ConditionBase {
  public async checkCondition(
    ...
    try {
      ...
      ErrorUtil.throwError(
        `Error checking condition ${RisingDoubleBottomConditionInfo.name}`,
        error
      );
    }
  }
  ...
  private detectRisingDoubleBottomPattern(candles: any[]): boolean {
    ...
    for (let i = 0; i < valleys.length - 1; i++) {
      ...
      if (heightDiff < 0.005 || heightDiff > 0.1) {
        continue;
      }
