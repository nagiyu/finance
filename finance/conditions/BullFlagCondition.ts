...
export const BullFlagConditionInfo: ConditionInfo = {
  name: 'ブルフラッグ',
  description:
    'ブルフラッグ（Bull Flag）は、テクニカル分析における有名な継続パターンの一つで、上昇トレンドの中で一時的な調整を挟んだ後、再び上昇が続くと予想されるチャート形状です。フラッグポール（急騰部分）の後に、やや下向きまたは横ばいの小幅な調整（フラッグ部分）が続き、その後上方向にブレイクアウトすることで新たな買いシグナルとなります。一般的に、目標価格はフラッグポールの長さをブレイク地点に加算して算出されます。',
  isBuyCondition: true,
...
export default class BullFlagCondition extends ConditionBase {
   ...
   */
  private findFlagpoleCandidates(
    candles: any[]
  ): Array<{ startIndex: number; endIndex: number; startPrice: number; endPrice: number }> {
    ...
    for (let i = 0; i < candles.length - 8; i++) {
      for (let j = i + 3; j <= Math.min(i + 8, candles.length - 5); j++) {
        ...
        if (gain >= 0.03) {
          ...
          let upwardCount = 0;
          for (let k = i; k < j; k++) {
            if (candles[k + 1].data[1] > candles[k].data[1]) {
              // close[k+1] > close[k]
              upwardCount++;
            }
          }
   ...
   */
  private findFlagCandidates(
    candles: any[],
    flagStart: number
  ): Array<{ startIndex: number; endIndex: number; highPrice: number; lowPrice: number }> {
    const candidates: Array<{
      startIndex: number;
      endIndex: number;
      highPrice: number;
    ...
    for (let i = breakoutStart; i < breakoutEnd; i++) {
      ...
      if (candleHigh > breakoutThreshold && candleClose > flag.highPrice) {
        ...
        const progressTowardTarget =
          (candleClose - flag.lowPrice) / (flagpole.endPrice - flag.lowPrice);

        if (progressTowardTarget >= 0.2) {
          // At least 20% progress toward flagpole high (more lenient)
          return true;
        }
      }
    }
