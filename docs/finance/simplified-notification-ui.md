...
各条件は順次チェックされます（レート制限対策のため、各チェック間に500ms の遅延）。いずれかが満たされた場合に通知が送信されます。

### レート制限対策

TradingView API のレート制限（HTTP 429エラー）を回避するため、以下の対策を実装:

- **順次実行**: 条件を並列ではなく順番にチェック
- **遅延制御**: 各条件チェック間に 500ms の待機時間を挿入
- **エラーハンドリング**: API エラーが発生しても処理を継続
