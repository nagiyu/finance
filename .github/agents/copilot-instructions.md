...
## Active Technologies
- Node.js 18.x / TypeScript 5.x (`strict` を有効) + Next.js (client)、TypeScript、`typescript-common`、`nextjs-common`、Jest、ESLint/Prettier、GitHub Actions (003-restructure-monorepo)
...
- 003-restructure-monorepo: Added Node.js 18.x / TypeScript 5.x (`strict` を有効) + Next.js (client)、TypeScript、`typescript-common`、`nextjs-common`、Jest、ESLint/Prettier、GitHub Actions

- 001-restructure-docs: Added Repository contains TypeScript (Node 18+/TS 5.x) and Markdown docs. Code language decisions for this feature: N/A for runtime, but tooling is Node/TypeScript based. + Existing repo uses `typescript-common` and `nextjs-common` submodules; docs tooling proposals: `remark`/`remark-lint`, `markdownlint`, and `prettier` for formatting.
