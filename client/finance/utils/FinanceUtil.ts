...
import { CandleStickData } from '@client-common/components/echarts/CandleStick';
...
export default class FinanceUtil {
  ...
  public static async getStockPriceData(exchange: string, ticker: string): Promise<CandleStickData[]> {
    const market = `${exchange}:${ticker}`;

    const result = await new Promise<CandleStickData[]>((resolve, reject) => {
