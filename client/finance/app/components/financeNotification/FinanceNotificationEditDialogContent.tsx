...
export default function FinanceNotificationEditDialogContent({
  ...
  const handleCreateCondition = async (condition: FinanceNotificationCondition) => {
    ...
    onItemChange({
      ...item,
      conditionList: [...item.conditionList, condition],
    });
    setEditConditionDialogOpen(false);
  };

  const handleUpdateCondition = async (condition: FinanceNotificationCondition) => {
    const conditionList = item.conditionList.map((c) => (c.id === condition.id ? condition : c));
    onItemChange({
      ...item,
  ...
  const handleDeleteCondition = async (condition: FinanceNotificationCondition) => {
    const conditionList = item.conditionList.filter((c) => c.id !== condition.id);
    onItemChange({
      ...item,
      conditionList: conditionList,
    });
  ...
  const conditionListToTable = (
    conditions: FinanceNotificationCondition[]
  ): FinanceNotificationConditionTableType[] => {
  ...
  return (
    <>
      <BasicSelect
        ...
        onChange={(value) => {
          ...
          onItemChange({
            ...item,
            exchangeId: value,
            tickerId: filteredTickers.length > 0 ? filteredTickers[0].id : '',
          });
          onStateChange({ ...state, filteredTickers });
        }}
      />
      ...
      <BasicTable
        columns={columns}
        data={conditionListToTable(item.conditionList)}
        loading={loading}
      />
      ...
      <BasicDialog
        ...
        onConfirm={
          isNewCondition
            ? () => handleCreateCondition(condition)
      ...
      >
        {() => (
          <>
            {conditionError && <ErrorAlert message={conditionError} />}
            <div>
              {ConditionClientUtil.formatCondition(condition.conditionName)}を削除しますか？
            </div>
          </>
        )}
      </BasicDialog>
