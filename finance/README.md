...
本モジュールはシステム全体で共有される金融データ処理の中核ロジックを担当します。クライアント（`client/finance`）やサーバー（`server/finance`）から利用される共通機能を提供し、ビジネスロジックの重複を防ぎます。

**このモジュールが担当すること:**

- 株価データの取得・加工ロジック（TradingView API 統合）
- 価格条件に基づくアラート・通知判定ロジック
- 取引所・ティッカー情報の管理
- 金融データの永続化処理
- 共通のインターフェース・型定義
...
- Lambda 関数のエントリーポイントやデプロイ設定 → `server/finance` が担当
- AWS サービスとの直接的なやり取り → `server/` 配下が担当
