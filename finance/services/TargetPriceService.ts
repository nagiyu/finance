...
export default class TargetPriceService {
  ...
  private static convertCurrency(
    ...
    if (sourceCurrency === 'USD' && targetCurrency === 'JPY') {
      exchangeRate = CurrencyUtil.getUsdToJpyRate();
      convertedAveragePrice = CurrencyUtil.convertUsdToJpy(result.averagePrice);
      convertedBuyTargetPrice = CurrencyUtil.convertUsdToJpy(result.buyTargetPrice);
      convertedSellTargetPrice = CurrencyUtil.convertUsdToJpy(result.sellTargetPrice);
    } else if (sourceCurrency === 'JPY' && targetCurrency === 'USD') {
      exchangeRate = CurrencyUtil.getJpyToUsdRate();
      convertedAveragePrice = CurrencyUtil.convertJpyToUsd(result.averagePrice);
      convertedBuyTargetPrice = CurrencyUtil.convertJpyToUsd(result.buyTargetPrice);
      convertedSellTargetPrice = CurrencyUtil.convertJpyToUsd(result.sellTargetPrice);
    } else {
