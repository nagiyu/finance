...
本アプリケーションはユーザー向けの Web インターフェースを担当します。株価データの可視化、アラート管理、ユーザー認証など、フロントエンド側のすべての機能を提供します。

**このモジュールが担当すること:**

- 株価チャートの表示とインタラクティブな操作（ECharts）
- ユーザー認証・セッション管理（NextAuth.js）
- 価格アラート・通知の表示とダッシュボード UI
- サーバーサイド API との連携
- レスポンシブなユーザーインターフェース
...
- Lambda 関数の実装やバックエンド処理 → `server/finance` が担当
