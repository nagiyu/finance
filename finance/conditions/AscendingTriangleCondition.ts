...
export const AscendingTriangleConditionInfo: ConditionInfo = {
  ...
  description: 'アセンディング・トライアングル（Ascending Triangle）は、上昇型三角持ち合いと呼ばれる強気の継続パターンです。価格が徐々に上昇している下値支持線と、ほぼ水平な上値抵抗線が収束する三角形の形を作ります。買い勢力が徐々に強まっていることを示し、最終的に上値抵抗線を上抜けることで強い上昇トレンドが期待されます。パターンの完成には水平抵抗線での複数回の跳ね返り、段階的に切り上がる安値、そして抵抗線の上方ブレイクアウトが必要です。',
  isBuyCondition: true,
...
export default class AscendingTriangleCondition extends ConditionBase {
  public async checkCondition(
    ...
    try {
      ...
      ErrorUtil.throwError(`Error checking condition ${AscendingTriangleConditionInfo.name}`, error);
    }
  }
   ...
   */
  private calculateSupportTrend(lows: Array<{index: number, price: number}>): {slope: number, intercept: number} | null {
    if (lows.length < 2) {
      return null;
    }
