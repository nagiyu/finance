...
export const BearCollarConditionInfo: ConditionInfo = {
  name: 'ベアコラッグ',
  description:
    'ベアコラッグ（Bear Collar）は、保有株式の下値リスクを制限しつつ上昇利益を一定範囲に制限するヘッジ戦略です。株価が急落した際の損失を限定し、同時にオプションプレミアムでコストを抑制することができます。このパターンは保有株式の価格が設定した下限（プット行使価格）に近づいた時や、上限（コール行使価格）に近づいた時に通知されます。',
  ...
  public async checkCondition(
    ...
    try {
      ...
      for (const candle of recentCandles) {
        if (candle.data[1] < candle.data[0]) {
          // close < open (bearish)
          bearishCandles++;
        }
      }
      ...
      const approachingBoundary =
        (currentPrice - minPrice) / range < 0.2 || (maxPrice - currentPrice) / range < 0.2;
