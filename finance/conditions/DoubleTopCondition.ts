...
export const DoubleTopConditionInfo: ConditionInfo = {
  name: 'ダブルトップ',
  description:
    'ダブルトップは、株価チャートに現れる代表的な天井圏のチャートパターンで、トレンド転換のシグナルとしてよく使われます。上昇トレンドの終盤で株価がほぼ同じ水準で2回高値をつけることで形成され、2つの山の間にできる安値ライン（ネックライン）を下抜けると下落トレンドへの転換シグナルとされます。',
...
export default class DoubleTopCondition extends ConditionBase {
  ...
  private detectDoubleTopPattern(candles: any[]): boolean {
    ...
    for (let i = 0; i < peaks.length - 1; i++) {
      ...
      const heightDiff =
        Math.abs(firstPeak.price - secondPeak.price) / Math.max(firstPeak.price, secondPeak.price);
      if (heightDiff > 0.05) {
        continue;
      }
